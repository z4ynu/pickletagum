...
To enable the optional availability assistant, also add `OPENAI_API_KEY`. This is server-only: do not give it a `PUBLIC_` prefix and never place it in browser code. The assistant checks public information only; it does not make reservations, payments, or guarantee a court is available.
...
3. To enable the availability assistant, add `OPENAI_API_KEY` (and optionally `OPENAI_AVAILABILITY_MODEL`) to the Production environment.
